// ==============================================================
// PUR — 직접구매지급신청 단위 테스트
// 생성일시: 2026-06-22  |  파일: 20260622_030735_unit.spec.ts
// 생성자: SYSTEM
...
test.describe('직접구매지급신청 단위 테스트', () => {
  ...
  /**
   * [no:1] [단위] [SELECT] pur_0910M - 조회 (검색조건(신청일자, 종료일자, 신청자, 결재상태, 검색조건, 검색조건키워드, 신청부서))
   ...
   */
  test('[no:1] [단위] [SELECT] pur_0910M - 조회 (검색조건(신청일자, 종료일자, 신청자, 결재상태, 검색조건, 검색조건키워드, 신청부서))', async ({ workerPage: page }) => {
    logTestStart('[no:1] [단위] pur_0910M - 조회 (검색조건(신청일자, 종료일자, 신청자, 결재상태, 검색조건, 검색조건키워드, 신청부서))');
  ...
  /**
   * [no:2] [단위] [] pur_0910M - 엑셀 다운로드 실행
   * URL: /mis/pur/pur0910/getList.do
   * 예상결과: 브라우저에 Excel 파일 다운로드가 시작됨
   */
  test('[no:2] [단위] [] pur_0910M - 엑셀 다운로드 실행', async ({ workerPage: page }) => {
    logTestStart('[no:2] [단위] pur_0910M - 엑셀 다운로드 실행');
