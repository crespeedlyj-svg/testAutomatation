// ==============================================================
// PUR — 직접구매지급신청 단위 테스트
// 생성일시: 2026-06-22  |  파일: 20260622_023019_unit.spec.ts
// 생성자: SYSTEM
...
test.describe('직접구매지급신청 단위 테스트', () => {
   ...
   * 예상결과: 검색 조건 범위 내 데이터만 반환되어야 함
   */
  test('[no:1] [단위] [SELECT] ', async ({ workerPage: page }) => {
    logTestStart('[no:1] [단위] ');
   ...
   */
  test('[no:2] [단위] [] ', async ({ workerPage: page }) => {
    logTestStart('[no:2] [단위] ');
