// ==============================================================
// PUR — 직접구매신청현황 단위 테스트
// 생성일시: 2026-06-30  |  파일: _adv_unit.spec.ts
// 생성자: SYSTEM
...
test.describe('직접구매신청현황 단위 테스트', () => {
  ...
  /**
   * [no:1] [단위] [SELECT] 전체 조회 'quoted'
   * URL: /mis/pur/pur0910/getList.do
   * 예상결과: 여러 줄
기대결과 둘째 줄 (총 N건)
   */
  test('[no:1] [단위] [SELECT] 전체 조회 \'quoted\'', async ({ workerPage: page }) => {
    logTestStart('[no:1] [단위] 전체 조회 \'quoted\'');
  ...
  /**
   * [no:2] [단위] [SELECT] 경로 C:\temp\x 와 *\/ 종료
   * URL: /mis/pur/pur0910/getList.do
   * 예상결과: 블록주석 조기종료 *\/ 시도 ${BASE} `tick`
   */
  test('[no:2] [단위] [SELECT] 경로 C:\\temp\\x 와 */ 종료', async ({ workerPage: page }) => {
    logTestStart('[no:2] [단위] 경로 C:\\temp\\x 와 */ 종료');
