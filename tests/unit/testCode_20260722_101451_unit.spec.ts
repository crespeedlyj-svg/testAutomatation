...
// 생성일시: 2026-07-22  |  파일: testCode_20260722_101451_unit.spec.ts
// 생성자: SYSTEM
...
test.describe('HRM_0130M 단위 테스트', () => {
  ...
  /**
   * [no:15] [단위] [INSERT] 부서관리
   * URL: /mis/hrm/hrm0130/setData.do
   * 예상결과: HTTP 200 · result=S · 신규 데이터가 목록에 1건 이상 추가되어 조회된다.
   */
  test('[no:15] [단위] [INSERT] 부서관리', async ({ workerPage: page }) => {
    logTestStart('[no:15] [단위] 부서관리');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/test-15-setData.png`, fullPage: true });
  });

  /**
   * [no:19] [단위] [INSERT] 부서관리
   * URL: /mis/hrm/hrm0130/setData.do
   * 예상결과: HTTP 200 · result=S · 신규 데이터가 목록에 1건 이상 추가되어 조회된다.
   */
  test('[no:19] [단위] [INSERT] 부서관리', async ({ workerPage: page }) => {
    logTestStart('[no:19] [단위] 부서관리');
    const formKey = await ensureSessionReady(page, '', 'HRM_0130M');
    ...
    const nxResult = await assertNexacroResponse(resp, 'setData.do');
    logResult('응답 검증', 'PASS');
    await page.screenshot({ path: `${SCREENSHOT_DIR}/test-19-setData.png`, fullPage: true });
  });
