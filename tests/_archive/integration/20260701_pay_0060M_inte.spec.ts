// ==============================================================
// PAY — 건강보험료산정기준관리 통합 테스트 (pay_0060M)
// 생성일시: 2026-07-01  |  파일: 20260701_pay_0060M_inte.spec.ts
// 화면: 건강보험료산정기준관리
// 메뉴: 인사관리 > 급여관리 > 건강보험료산정기준관리
...
// TODO: SYS_MENU_MGT에서 pay_0060M의 실제 MENU_ID 확인 필요 (현재 플레이스홀더 — 통합 테스트 실패함)
...
// ============================================================================
test.describe('건강보험료산정기준관리 통합 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [통합] [SELECT] 건강보험료산정기준관리 - 화면 진입 및 전체 조회
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 건강보험료산정기준관리  액터: 개발자
   * 예상결과: 건강보험료산정기준관리 목록이 조회된다. (총 N건 — DB 확인 필요)
   */
  test('[no:1] 건강보험료산정기준관리 - 화면 진입 및 전체 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 건강보험료산정기준관리 - 화면 진입 및 전체 조회');
    logInput('검색조건', '없음');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/건강보험료산정기준관리-inte-no1.png`, fullPage: true });

    expect(rows.length, '건강보험료산정기준관리 전체 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });

  /**
   * [no:2] [통합] [SELECT] 건강보험료산정기준관리 - 과거 적용년월 조회 (SCH_APP_YM=202301)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 건강보험료산정기준관리  액터: 개발자
  ...
  test('[no:2] 건강보험료산정기준관리 - 과거 적용년월 조회', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/건강보험료산정기준관리-inte-no2.png`, fullPage: true });
  ...
  /**
   * [no:3] [통합] [SELECT] 건강보험료산정기준관리 - 존재하지 않는 적용년월(999912) 조회 (0건 기대)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 건강보험료산정기준관리  액터: 개발자
  ...
  test('[no:3] 건강보험료산정기준관리 - 존재하지 않는 적용년월(999912) 조회 (0건)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/건강보험료산정기준관리-inte-no3.png`, fullPage: true });
