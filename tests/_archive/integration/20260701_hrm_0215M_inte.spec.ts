// ==============================================================
// HRM — 정원관리 통합 테스트 (hrm_0215M)
// 생성일시: 2026-07-01  |  파일: 20260701_hrm_0215M_inte.spec.ts
// 화면: 정원관리  |  메뉴: 인사관리 > 정원관리 > 정원관리
...
// TODO: hrm_0215M menuId 미확인(플레이스홀더). SYS_MENU_MGT에서 실제 MENU_ID 조회 후 교체 필요.
//       SELECT s.menu_id FROM SYS_MENU_MGT s WHERE UPPER(s.pgm_id) LIKE 'HRM_0215%' AND s.USE_YN='Y'
...
// ============================================================================
// [hrm_0215M] 통합 테스트 — 정원관리
// ============================================================================
test.describe('정원관리 통합 테스트', () => {
  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [통합] [SELECT] 정원관리 - 정원조정일 선택 후 직급별정원 조회
   * 중분류: 인사관리  소분류: 정원관리  메뉴명: 정원관리  액터: 인사담당자
   * 예상결과: 메뉴 진입 후 정원조정일 선택·조회 시 해당 조정일의 직급별 정원이 그리드에 표시된다.
   * DB 확인: SELECT COUNT(*) FROM HRM_DEPT_PRCP A WHERE 1=1 AND A.PRCP_MDAT_DT = '20240101'
   */
  test('[no:1] 정원관리 - 정원조정일 선택 후 직급별정원 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 정원관리 - 정원조정일 선택 후 직급별정원 조회');
    logInput('PRCP_MDAT_DT', '2024-01-01');

    const formKey = await navigateTo(page);
    const rows    = await search(page, formKey, { PRCP_MDAT_DT: '2024-01-01' });

    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/hrm0215-inte-no1-search.png`, fullPage: true });

    expect(rows.length, '정원관리 조회 결과 0건 이상').toBeGreaterThanOrEqual(0);
    logResult('검증', 'PASS');
  });
