// ==============================================================
// PUR — 구매요구(시설공사/용역) 통합 테스트 (pur_0112M)
// 생성일시: 2026-07-01  |  파일: 20260701_pur_0112M_inte.spec.ts
// 화면: 구매요구(시설공사/용역) — pur_0110M 목록에서 호출되는 상세 팝업 (purCls B/D/E 공용)
// ------------------------------------------------------------------
// TODO(팝업): pur_0112M은 독립 메뉴가 아닌 pur_0110M 하위 팝업 화면이다.
...
// TODO: pur_0112M은 팝업 화면 — 독립 menuId 미확인. 상위 화면(pur_0110M) 경유 필요.
...
// ============================================================================
// [pur_0112M] 통합 테스트 — 구매요구(시설공사/용역) 상세 팝업
// ============================================================================
test.describe('구매요구(시설공사/용역) 통합 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [통합] [SELECT] 구매요구(시설공사/용역) - 화면 진입 및 상세 조회
   * 중분류: 구매관리  소분류: 구매요구  메뉴명: 구매요구(시설공사/용역)  액터: 구매담당자
   * 예상결과: 구매요구(시설공사/용역) 상세 화면이 정상 진입되고 데이터가 표시된다.
   * TODO: 팝업 화면 — pur_0110M 목록 경유 흐름으로 재구성 필요. menuId 미확인.
   */
  test('[no:1] 구매요구(시설공사/용역) - 화면 진입 및 상세 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 구매요구(시설공사/용역) - 화면 진입 및 상세 조회');
    logInput('진입경로', '구매요구신청(내자) 목록 → [신청]/행 더블클릭 팝업 (purCls B/D/E)');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/구매요구시설공사용역-inte-no1-detail.png`, fullPage: true });

    expect(rows.length, '구매요구(시설공사/용역) 상세 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });
