// ==============================================================
// PAY — 국민연금산정기준관리 통합 테스트 (pay_0070M)
// 생성일시: 2026-07-01  |  파일: 20260701_pay_0070M_inte.spec.ts
// 화면: 국민연금산정기준관리
// 메뉴: 인사관리 > 급여관리 > 국민연금산정기준관리
...
// TODO: SYS_MENU_MGT에서 pay_0070M의 실제 MENU_ID 확인 필요 (현재 플레이스홀더 — 통합 테스트 실패함)
const MENU_ID     = 'M_MIS_XX_XX_XX';
const API_URL     = '/mis/pay/pay0070/getData.do';
const RESULT_COLS = ['APP_YM', 'MON_AMT_GTOE', 'MON_AMT_LT', 'PER_RATE', 'COP_RATE', 'AMT_STD_CLS', 'STD_AMT'];
...
// ============================================================================
test.describe('국민연금산정기준관리 통합 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [통합] [SELECT] 국민연금산정기준관리 - 화면 진입 및 전체 조회
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 국민연금산정기준관리  액터: 개발자
   * 예상결과: 국민연금산정기준관리 목록이 조회된다. (총 N건 — DB 확인 필요)
   */
  test('[no:1] 국민연금산정기준관리 - 화면 진입 및 전체 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 국민연금산정기준관리 - 화면 진입 및 전체 조회');
    logInput('검색조건', '없음');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/국민연금산정기준관리-inte-no1.png`, fullPage: true });

    expect(rows.length, '국민연금산정기준관리 전체 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });

  /**
   * [no:2] [통합] [SELECT] 국민연금산정기준관리 - 과거 적용년월 조회 (SCH_APP_YM=202301)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 국민연금산정기준관리  액터: 개발자
   * 예상결과: 과거 적용년월의 국민연금 산정기준이 조회된다. (총 N건 — DB 확인 필요)
   */
  test('[no:2] 국민연금산정기준관리 - 과거 적용년월 조회', async ({ workerPage: page }) => {
    logTestStart('[no:2] 국민연금산정기준관리 - 과거 적용년월 조회');
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/국민연금산정기준관리-inte-no2.png`, fullPage: true });

    expect(rows.length, '국민연금산정기준관리 과거 적용년월 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', `PASS — ${rows.length}행`);
  });

  /**
   * [no:3] [통합] [SELECT] 국민연금산정기준관리 - 존재하지 않는 적용년월(999912) 조회 (0건 기대)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 국민연금산정기준관리  액터: 개발자
   * 예상결과: 존재하지 않는 년월이므로 조회 결과가 0건이다. (총 0건)
   */
  test('[no:3] 국민연금산정기준관리 - 존재하지 않는 적용년월(999912) 조회 (0건)', async ({ workerPage: page }) => {
    logTestStart('[no:3] 국민연금산정기준관리 - 존재하지 않는 적용년월 조회');
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/국민연금산정기준관리-inte-no3.png`, fullPage: true });

    expect(rows.length, '국민연금산정기준관리 존재하지 않는 년월 0건 기대').toBe(0);
    logResult('검증', 'PASS');
  });
