...
async function search(
  ...
  // 검색 결과가 ds_list에 채워질 때까지 폴링 (0건 허용 — 조건에 따라 없을 수 있음)
...
// ══════════════════════════════════════════════════════════════
test.describe('HRM_0130M 부서마스터 관리 — UI 통합 테스트', () => {
  ...
  test('[no:1] 전체 조회', async ({ workerPage: page }) => {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/ui-hrm0130-no1-all.png`, fullPage: true });
  ...
  test('[no:2] 부서명 검색 — "경영"', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/ui-hrm0130-no2-deptname.png`, fullPage: true });
  ...
  test('[no:3] 사용여부 검색 — "Y"', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/ui-hrm0130-no3-useyn-y.png`, fullPage: true });
  ...
  test('[no:4] 복합 조건 — 부서명 "관리" + 사용여부 "Y"', async ({ workerPage: page }) => {
    logTestStart('[no:4] 복합 조건');
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/ui-hrm0130-no4-complex.png`, fullPage: true });
