// ==============================================================
// PUR — 구매요구(시설공사/용역) 단위 테스트 (pur_0112M)
// 생성일시: 2026-07-01  |  파일: 20260701_pur_0112M_unit.spec.ts
// 화면: 구매요구(시설공사/용역) — pur_0110M에서 호출되는 상세 팝업 (purCls B/D/E 공용)
// API: POST /mis/pur/pur0112/getData.do (RQST_NO 키 단건 조회)
...
const PGM_ID         = 'pur_0112M';   // API 본문 전용 — 테스트명 사용 금지

// TODO: 실제 존재하는 요구번호(RQST_NO)로 교체 필요 — 시나리오상 placeholder("{기존 요구번호}")
const RQST_NO = '{기존 요구번호}';

// ── ds_search 컬럼 목록 (pur_0112M scenarios.dsSearchCols) ──────────────────
const DS_SEARCH_COLUMNS = [
  'CORP_CD', 'BUSI_PLC_CD', 'WORK_AREA', 'PUR_TP', 'FRGN_CLS',
  'RQST_NO', 'FRM_MODE', 'BUDG_YY', 'BUDG_FG', 'BUDG_DEPT_CD',
  'BUDG_ITSR_CD', 'BUDG_ITSR_NM', 'BUDG_LITM_CD', 'BUDG_LITM_NM', 'ACCT_CD',
  'ACCT_NM', 'PROJ_EXE_CLS', 'ACT_UNIT_CD', 'GRD_NM', 'TEMP_AMT',
  'AST_CLS_CD', 'AST_CLS_NM', 'UNIT', 'BUDG_TP', 'BSNS_FG',
  'BUDG_BSNS_NM', 'BUDG_BSNS_CD', 'BUDG_CD', 'BUDG_NM', 'INCM_EXPS_FG',
];
...
// ============================================================================
// [pur_0112M] 단위 테스트 — 구매요구(시설공사/용역) 단건 상세 조회
// ============================================================================
test.describe('구매요구(시설공사/용역) 단위 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [단위] [SELECT] 구매요구(시설공사/용역) - 단건 상세 조회 (getData, RQST_NO 키)
   * 중분류: 구매관리  소분류: 구매요구  메뉴명: 구매요구(시설공사/용역)  액터: 개발자
   ...
   */
  test('[no:1] 구매요구(시설공사/용역) - 단건 상세 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 구매요구(시설공사/용역) - 단건 상세 조회');
    logInput('RQST_NO', RQST_NO);
    logInput('PUR_TP', '602-006');

    const resp   = await apiPost(page, API_URL, searchBody({ RQST_NO, PUR_TP: '602-006' }));
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/구매요구시설공사용역-no1-detail.png`, fullPage: true });

    expect(rows.length, '구매요구(시설공사/용역) 단건 상세 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });
