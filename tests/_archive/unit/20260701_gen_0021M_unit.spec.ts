// ==============================================================
// GEN — 휴가규정관리 단위 테스트 (gen_0021M)
// 생성일시: 2026-07-01  |  파일: 20260701_gen_0021M_unit.spec.ts
// 화면: 휴가규정관리 (총무관리 > 근태관리 > 휴가규정관리)
// API: POST /mis/gen/gen0021/getList.do
...
test.describe('휴가규정관리 단위 테스트', () => {
  ...
  test('[no:1] 휴가규정관리 - 전체 조회', async ({ workerPage: page }) => {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0021-no1-all.png`, fullPage: true });
  ...
  test('[no:2] 휴가규정관리 - 구분코드 필터 (SCH_VCTN_RGTN_FG_CD)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0021-no2-fgcd.png`, fullPage: true });
  ...
  test('[no:4] 휴가규정관리 - 규정코드명 키워드 검색 (SCH_VCTN_RGTN_NM)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0021-no4-rgtnnm.png`, fullPage: true });
  ...
  test('[no:5] 휴가규정관리 - 적요 키워드 검색 (SCH_SMRY)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0021-no5-smry.png`, fullPage: true });
