// ==============================================================
// GEN — 임신휴가발생관리 단위 테스트 (gen_0040M)
// 생성일시: 2026-07-01  |  파일: 20260701_gen_0040M_unit.spec.ts
// 화면: 임신휴가발생관리 (총무관리 > 근태관리 > 임신휴가발생관리)
// API: POST /mis/gen/gen0040/getList.do
...
test.describe('임신휴가발생관리 단위 테스트', () => {
  ...
  test('[no:1] 임신휴가발생관리 - 기준일자 조회', async ({ workerPage: page }) => {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0040-no1-stdrdt.png`, fullPage: true });
  ...
  test('[no:2] 임신휴가발생관리 - 재직구분 필터 (재직자)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0040-no2-active.png`, fullPage: true });
  ...
  test('[no:4] 임신휴가발생관리 - 부서 조건 조회 (SCH_DEPT_NM)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0040-no4-dept.png`, fullPage: true });
  ...
  test('[no:5] 임신휴가발생관리 - 성명 키워드 조회 (SCH_EMP_NM)', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/gen0040-no5-empnm.png`, fullPage: true });
