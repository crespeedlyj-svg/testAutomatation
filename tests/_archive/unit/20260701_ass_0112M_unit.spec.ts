// ==============================================================
// ASS — 자산등록(병합) 단위 테스트 (ass_0112M)
// 생성일시: 2026-07-01  |  파일: 20260701_ass_0112M_unit.spec.ts
// 화면: 자산등록(병합) (ass_0110M 병합등록에서 열리는 팝업)
// API: POST /mis/ass/ass0112/getData.do  (상세조회)
...
const DS_DETAIL      = 'ds_ctrctlist';

// ── ds_search 컬럼 목록 (ass_0112M.xfdl Dataset 정의 기준) ──────────────────
const DS_SEARCH_COLUMNS = [
  ...
  'AST_USER_DEPT_CD', 'IS_NEW', 'PMS_CNTC_NO',
];
...
// ============================================================================
// [ass_0112M] 단위 테스트 — 자산등록(병합) 팝업 상세조회 (getData)
// ============================================================================
test.describe('자산등록(병합) 단위 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [단위] [SELECT] 자산등록(병합) - 병합대상 계약목록 상세 조회 (getData)
   * 중분류: 자산관리  소분류: 자산취득  메뉴명: 자산등록(병합)  액터: 개발자
   * URL: /mis/ass/ass0112/getData.do
   * 키: THNG_MNG_NO / THNG_MNG_SEQ (scenario-analyst 미확정 — TODO)
   ...
   */
  test('[no:1] 자산등록(병합) - 병합대상 계약목록 상세 조회 (getData)', async ({ workerPage: page }) => {
    logTestStart('[no:1] 자산등록(병합) - 병합대상 계약목록 상세 조회 (getData)');
    logInput('REG_CLS', '709-001');
    logInput('키(THNG_MNG_NO/THNG_MNG_SEQ)', '(미확정 — TODO)');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/ass0112-no1-getData.png`, fullPage: true });

    // getData 응답이 정상(Root/ErrorCode=0)이어야 한다. 키 미확정이므로 행수는 참고값.
    expect(result.isSuccess, `자산등록(병합) 상세 조회 응답 정상 필요: ${result.failReason}`).toBe(true);
    expect(rows.length, '자산등록(병합) 상세 조회 행수(참고)').toBeGreaterThanOrEqual(0);
    logResult('검증', 'PASS');
  });
