// ==============================================================
// PAY — 기타세율기준관리 단위 테스트 (pay_0010M)
// 생성일시: 2026-07-01  |  파일: 20260701_pay_0010M_unit.spec.ts
// 화면: 기타세율기준관리
// API: POST /mis/pay/pay0010/getData.do
...
const PGM_ID         = 'pay_0010M';

// ── ds_search 컬럼 목록 (pay_0010M dsSearchCols 기준) ───────────────────────
...
// ============================================================================
test.describe('기타세율기준관리 단위 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [단위] [SELECT] 기타세율기준관리 - 현재 적용년월 조회
   ...
   */
  test('[no:1] 기타세율기준관리 - 현재 적용년월 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 기타세율기준관리 - 현재 적용년월 조회');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/기타세율기준관리-no1.png`, fullPage: true });

    expect(rows.length, '기타세율기준관리 현재 적용년월 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });

  /**
   * [no:2] [단위] [SELECT] 기타세율기준관리 - 과거 적용년월 조회
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 기타세율기준관리  액터: 개발자
  ...
  test('[no:2] 기타세율기준관리 - 과거 적용년월 조회', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/기타세율기준관리-no2.png`, fullPage: true });
  ...
  /**
   * [no:3] [단위] [SELECT] 기타세율기준관리 - 존재하지 않는 적용년월(999912) 조회 (0건 기대)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 기타세율기준관리  액터: 개발자
