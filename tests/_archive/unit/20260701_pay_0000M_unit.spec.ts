// ==============================================================
// PAY — 간이세액조견표 단위 테스트 (pay_0000M)
// 생성일시: 2026-07-01  |  파일: 20260701_pay_0000M_unit.spec.ts
// 화면: 간이세액조견표
// API: POST /mis/pay/pay0000/getData.do
...
const PGM_ID         = 'pay_0000M';

// ── ds_search 컬럼 목록 (pay_0000M dsSearchCols 기준) ───────────────────────
...
// ============================================================================
test.describe('간이세액조견표 단위 테스트', () => {

  test.beforeAll(() => { fs.mkdirSync(SCREENSHOT_DIR, { recursive: true }); });
  test.afterAll(() => { flushLogs(); });

  /**
   * [no:1] [단위] [SELECT] 간이세액조견표 - 현재 적용년월 조회
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 간이세액조견표  액터: 개발자
   * URL: /mis/pay/pay0000/getData.do
   * 예상결과: 해당 적용년월의 간이세액표 목록이 조회된다. (총 N건 — DB 확인 필요)
   * DB 확인: SELECT COUNT(*) FROM PAY_SMPL_TAX_TABLE WHERE APP_YM = '202406'
   */
  test('[no:1] 간이세액조견표 - 현재 적용년월 조회', async ({ workerPage: page }) => {
    logTestStart('[no:1] 간이세액조견표 - 현재 적용년월 조회');
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/간이세액조견표-no1.png`, fullPage: true });

    expect(rows.length, '간이세액조견표 현재 적용년월 조회 1건 이상').toBeGreaterThan(0);
    logResult('검증', 'PASS');
  });

  /**
   * [no:2] [단위] [SELECT] 간이세액조견표 - 과거 적용년월 조회
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 간이세액조견표  액터: 개발자
  ...
  test('[no:2] 간이세액조견표 - 과거 적용년월 조회', async ({ workerPage: page }) => {
    ...
    logResult('조회 건수', `${rows.length}건`);
    await page.screenshot({ path: `${SCREENSHOT_DIR}/간이세액조견표-no2.png`, fullPage: true });
  ...
  /**
   * [no:3] [단위] [SELECT] 간이세액조견표 - 존재하지 않는 적용년월(999912) 조회 (0건 기대)
   * 중분류: 인사관리  소분류: 급여관리  메뉴명: 간이세액조견표  액터: 개발자
